...
usage: "Paste into a NEW Claude Code session that has org-wide GitHub access. Run the orchestrator on Opus (fast mode); it fans out Sonnet for static sites and Opus for any repo with a real build to get green."
model_guidance:
...
## STEP 3 — EXECUTE (one sub-agent per actionable repo; parallelize)
Model per sub-agent: **Sonnet 5** for Track C and simple Track A; **Opus 4.8** for any Track A repo with a
real build/test to get green. Give each sub-agent the full SPAWN-REPO-HYGIENE-ROLLOUT text with TARGET
filled (TARGET_NAME, TARGET_PATH=the repo, BRANCH=`chore/repo-hygiene-rollout`) and its assigned TRACK.
