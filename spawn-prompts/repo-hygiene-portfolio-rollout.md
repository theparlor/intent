...
usage: "Don't paste this whole file — it's committed to GitHub repo theparlor/intent at spawn-prompts/repo-hygiene-portfolio-rollout.md. In a NEW org-scoped session (Opus, fast mode) tell it: `Fetch spawn-prompts/repo-hygiene-portfolio-rollout.md + its two GROUND TRUTH files from theparlor/intent, then execute the portfolio rollout against the theparlor org; show me the STEP 2 inventory and wait before STEP 3.` NOTE: until this lands on the default branch, add ref=<branch> to the fetch (currently claude/mobai-team-practices-yjd0qg). Once merged to main, no ref needed. Local sessions: the repo root is <Workspaces>/Core/frameworks/intent, not `theparlor/intent` — that's the GitHub owner/repo, not a filesystem path."
model_guidance:
