...
Disposition: mixed, one layer overtaken, two layers still pending, Brien-gated. The Phase (a) design doc landed (`spec/2026-04-21-rigor-audit-program-design.md` exists). Layer D (export redaction) is now overtaken by events: the 2026-06-10 no-preemptive-redaction policy (`memory/feedback_no_preemptive_redaction.md`, "Brien's access = unredacted everywhere; redaction only at public boundary") rejected exactly the automated redaction/classifier pipeline `SHARING_AND_REDACTION_SCHEMA.md` proposed; confirmed no `redacted_export.py` or `AUDIENCES.yml` exist and none are planned. But Layer B (access control) and Layer C (audit + signal), this signal's own named "real gaps," remain unbuilt: no declared credentials/rights/permissions model for cross-engagement knowledge-farm access, and no query-log/content-discovery-signal pipeline exists. This is a genuine architectural gap, not a symptom-repair; not registering a new row in `Workspaces/.context/PENDING_DECISIONS.md` (read-only for this pass) but flagging that Layer B/C access-control design belongs there if Brien wants it prioritized.
