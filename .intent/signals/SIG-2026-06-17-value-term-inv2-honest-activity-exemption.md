...
catch_mechanism: INV-VALUE-TERM-CLEAN remains armed and unchanged in scope. The exemption is additive — it can only convert a FAIL→PASS for entries matching the documented honest-disclosure convention; it cannot newly-pass a SILENT activity term (no outcome_signal) nor one that CLAIMS an outcome_signal while measuring activity (those still FAIL). Zero-violation-start verified: full cross-product registry run exits 0 on 2026-06-17 (INV-VALUE-TERM-CLEAN PASS, INV-VALUE-TERM-COVERAGE PASS).
