...
upstream_control_path: "bin/ CLI suite (intent-signal, intent-intent, intent-spec, intent-status, intent-init); tools/intent-mcp/server.py; .github/workflows/intent-events.yml"
catch_mechanism: "The installable product this signal asked for exists: MCP server plus CLI plus GitHub Action, matching CLAUDE.md's own 'Notice (Operational)' maturity marker"
verification_command: "ls /Users/brien/Workspaces/Core/frameworks/intent/bin/"
---
...
Disposition: control exists now. bin/intent-init plus the rest of the CLI suite, the MCP server, and the GitHub Action are exactly the installable path this signal was asking for. A practitioner can install and run Intent on their own repo today without reading the methodology essay first.
