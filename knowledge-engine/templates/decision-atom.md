...
> **In plain terms:** A decision atom is the one-page record of a call that's already been made and does not need Brien's sign-off: what was decided, why, and what was ruled out. Use it any time a reversible, workspace-local choice needs a paper trail so nobody re-litigates it later or forgets why it happened. Below is the 4-question checklist and the copy-paste format agents use to write and file it correctly.
...
upstream_control_path: [path to the mechanism that enforces this decision going forward, e.g. spec section, invariant ID, chain_audit check, hook, or schema field]
...
[Why this decision is needed now. What forces are at play. Reference the spec section, trigger signal, or build artifact that raised this question. 3 to 5 sentences max.]
