...
| `signal-recorder-silent-check.sh` | PreToolUse (Write/Edit) | WS-DDR-098 recorder check, amended by WS-DDR-150: a product that declares autonomy settings but sent Witness no event in 30 days, or whose events mostly lack the minimum fields, gets a once-per-session note in the model's context (warn-only); every fire reports itself to Witness as `hook.fire` |
...
- **No actions of its own.** A product with nothing that acts (a content or methodology repo: markdown only, no scripts, hooks, jobs or servers) declares `witness_actions: none (<named reason>)` at column 0 in `.intent/INTENT.md`, outside the managed lambda_settings block. The reason in parentheses is required; a bare `none` is not a declaration. The report shows the product as `no-actions` with its reason, and the hook skips it with telemetry detection `no-actions-declared`. Events still win: a product that declares no actions but has events in the window reads as active. Added for the WS-DDR-150 backfill (QMT-01M3F5JYA4643ANV7GPPRJDPZR).
- **Field lint (WS-DDR-150).** While it reads each event the builder checks it against the rule's minimum event: `ts` with a timezone, `product`, `event`, and the attributes `caller`, `machine`, `outcome`, `duration_ms`, `run_id`, plus `redaction_level: client-confidential` when an attribute names a path under `Work/<kind>/Engagements/`. Presence is checked, not values, except for `redaction_level`. OTLP spans map span name, `service.name`, `startTimeUnixNano`, start and end times, `traceId` and `status.code` to their equivalents. The index keeps, per product over the same 30 days, `lint_30d` with events, how many miss a field, how often each field is missing, and the most common one. When more than half of an active product's rule-bound events miss a field, the hook tells the session once, in one sentence naming that field and pointing at `Core/products/witness/src/emit.py` (detection `nonconformant-events`). Added by QMT-01M3F5JWBR8XPK9YGB7HTMCBNP.
- **Legacy versus rule-bound.** Decided from each event, with no product list to keep current: an event is legacy when its `source_system` is `screenpipe`, `cc-native` or `entire-io` (capture channels in those tools' own formats), or when it carries `attributes.adapter` (Witness's stream adapters stamp it on every line they translate from an older log; the emit.py path never does). Everything else is rule-bound: events a program wrote itself. Legacy events are counted and shown by `--report` but never produce a session note. A product that reports only by appending to its `.intent/events` file arrives through an adapter and so is never linted; emit.py is the rule's path, and both notes say so.
- **Its own events.** Every hook fire reports itself through emit.py as product `intent`, event `hook.fire`, caller `hook:signal-recorder-silent-check`, with the detection, the product directory name as target (a hash plus `redaction_level: client-confidential` for an engagement path), the session id and its duration. Every builder run reports itself as `job.run` with `job: witness-source-index` and its counts; that is the lint pass's own event. Both are fail-open: quiet when emit.py is absent, one stderr line when it fails, and the check or build result stands. The bypass stays inert.
- **The index.** `$HOME/.claude/state/witness-source-index.json`, outside git. The builder reads only bytes appended since its last run (the first run reads the whole store, about 1.4 GB in 33 s with the field lint on 2026-09-26; a state written before the lint is re-read in full once), holds a lock, and rebuilds at most once a day unless run with `--force`. The hook never reads the store: when the index is missing or more than 24 hours old it starts the builder detached, at most once an hour, and answers from what it has. A missing or malformed index is recorded as `witness-index-missing` or `witness-index-malformed` and nothing is shown.
...
- **Commands.** `/usr/bin/python3 hooks/witness_source_index.py --report` prints every declaring product with the names checked, last Witness event, 30-day count and verdict, then the field conformance table: every Witness name with events in the window, rule-bound and legacy counts apart, the most common missing field, and whether a session would be told (`--json` for both). Tests: `hooks/tests/test_signal_recorder_silent_check.py` and `hooks/tests/test_witness_source_index.py`. Bypass: `SIGNAL_RECORDER_SILENT_BYPASSED=1`. Telemetry: `$HOME/.claude/logs/signal-recorder-silent.jsonl`.
