...
6. **Comparator authority at the seam**, the price of autonomy where the check is weak. The matrix's "deterministic precondition" column is binary: the hook fires or it does not. It carries no term for how much authority the seam's comparator has earned, and no term for what buying more authority costs. Intent already ranks comparators by `criteria_origin` in `spec/typed-evaluation-verdicts.md`: a `self` judge is cheap, same-lineage, and capped at inner-loop authority, while only a `derived` judge may close. The missing facet is that the gate should read that ladder, so a seam with no hard verifier is not barred from L3 but is quoted a price for it (a foreign judge or a human in the closure position, with the cheap self-graded judge ranking underneath). External evidence for the cheap end being genuinely useful: VibeThinker-3B's CLR loop, a same-lineage self-graded comparator consumed at ranking authority only, bought 2.7 to 6.1 benchmark points with zero added parameters. (Added 2026-09-16 from intake issue 18; rationale and the rejected stronger form in `knowledge/design-rationale/RAT-004-verifier-at-the-seam.md`. Not yet addressed in the flight-model spec.)
