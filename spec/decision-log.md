...
**Consequences:** The intent-knowledge envelope gains two fields (`sightline`, `supply_policy`) and two verbs (`get_core`, `audit_chain`); `query` is reused. A round-trip test enforces `preservation_invariant` (wired like a Voices INV-*). `supply_policy` composes with — does not replace — classification tier (DEC-010). **Built + tested 2026-05-31 (TDD):** D1–D4 + `preservation_invariant` implemented in `servers/knowledge.py` (verbs `get_core`/`audit_chain`, fields `sightline`/`supply_policy`); `servers/test_knowledge.py` green at **56 tests** (20 new). The remaining Execute step is the deploy (`intent-knowledge.fastmcp.cloud`), which is Brien-driven.
